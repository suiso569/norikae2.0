...
def get_station_kana():
    return {
        ...
        "蒼海宮": "そうかいぐう", "荒山村": "あらやまむら", "桜町一丁目": "さくらまちいっちょうme",
        ...
        for i in range(len(sorted_slines)):
            for j in range(i+1, len(sorted_slines)):
                ...
                if sta == "白浜崎" and set([line_a, line_b]) == {"桜町線(各停)", "桜町線(快速)"}:
                    ...
                    G.add_edge(f"{sta}|{line_a}", f"{sta}|{line_b}", weight=30, weight_trans=10000, line="乗り換え", is_transfer=True)
