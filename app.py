    ...
    for sta, slines in station_lines.items():
        ...
        for i in range(len(sorted_slines)):
            for j in range(i+1, len(sorted_slines)):
                ...
                # 白浜崎駅での桜町線（各停）⇔（快速）の直通特例
                if sta == "白浜崎" and set([line_a, line_b]) == {"桜町線(各停)", "桜町線(快速)"}:
                    G.add_edge(f"{sta}|{line_a}", f"{sta}|{line_b}", weight=0, line="直通", is_transfer=False)
                else:
                    G.add_edge(f"{sta}|{line_a}", f"{sta}|{line_b}", weight=30, line="乗り換え", is_transfer=True)
